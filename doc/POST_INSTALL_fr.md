...
Le endpoint Web (pour un accès public HTTP, pour un site Web statique ou des fichiers de médias sociaux) est __PORT_WEB__, et si vous voulez l'utiliser, vous devez autoriser le site web pour le bucket, et une application `redirect_ynh` pointant vers `127.0.0.1:__PORT_WEB__`, en utilisant le domaine `bucketname.__DOMAIN__`.
